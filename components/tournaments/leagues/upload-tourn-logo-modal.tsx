...
const UploadTournLogoModal = ({ open, setOpen }: UploadProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("userId", tournId);

    const res = await uploadImage(formData);

    // console.log(res.url);
    if (res.error === "0") {
      mutation.mutate({ url: res.url!, id: tournId });
    } else {
      toast({
        title: "Error!",
        variant: "destructive",
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChangeWrapper}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Upload Tournament Logo</DialogTitle>
          <DialogDescription>
            You are uploading tournament logo.
          </DialogDescription>
        </DialogHeader>
...
export default UploadTournLogoModal;
